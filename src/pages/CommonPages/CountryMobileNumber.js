...
import { countryListData, InputTypesEnum } from '../../config/commonVariable';

const CountryMobileNumber = ({ onChangeMobileNumber, onChangeCountryCode, setDefaultData, className, imageIcon, disabled }) => {
    ...
    const [inputValue, setInputValue] = useState(setDefaultData != undefined ? setDefaultData?.mobile_number : '')//setDefaultData?.mobile_number)  //setDefaultData != undefined && setDefaultData != '' ? setDefaultData?.mobile_number : '');

    const [selectedCountry, setSelectedCountry] = useState({
        id: 101,
        name: "India",
        flag: "https://eveksia.s3.me-central-1.amazonaws.com/flag/india.png",
        code: "IN",
        dial_code: "+91"
    });
    ...
    useEffect(() => {
        if (setDefaultData != undefined && setDefaultData != "") {
            ...
            const filtered = countryListData?.filter(country =>
                country?.dial_code?.toLowerCase().includes(setDefaultData?.country_code)
            );
    ...
    const handleSearchChange = (event) => {
        ...
        const filtered = countryListData?.filter(country =>
            ...
            || country?.dial_code?.toLowerCase().includes(value)
        );
        ...
        <>
            <div className='d-flex'>
                <div class="btn-group me-2">
                    ...
                    >
                        {(selectedCountry || countryListData) ? (
                            <>
                                <img src={selectedCountry?.flag || ''} className="flag-icon me-1 " style={{ borderRadius: '30px' }} />
                                <span className='me-1'>{selectedCountry?.dial_code}</span>
                            </>
                        ) : (
                            <span>Select</span>
                        )}
                        ...
                        {filteredCountries?.map(country => (
                            <li key={country.id}>
                                <a className="dropdown-item w-100" href="#" onClick={() => handleCountrySelect(country)}>
                                    <img src={country.flag} className="flag-icon pe-1" alt={`${country.dial_code}`} />
                                    {country.dial_code}
                                </a>
                            </li>
                        ))}
                ...
                <div className="input-group border rounded-1">
                    ...
                    <input type="text" value={inputValue} disabled={disabled} className={`form-control  ${imageIcon ? 'border-0' : ''} ps-2 `} placeholder={'Enter mobile number'}
