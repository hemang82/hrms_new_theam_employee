...
export default function ApplicationContent(props) {
    ...
    const screenOptions = [
        ...
        { label: "Terms And Conditions", value: "terms_condition" }
    ];
    ...
    useEffect(() => {
        detailsCmsPages({ type: selectedScreen?.value }).then((response) => {
            if (response?.code === Codes?.SUCCESS) {
                setEnglishContent(response?.data[0]?.description)
                setHindiContent(response?.data[0]?.hi_description)
                setGujratiContent(response?.data[0]?.gu_description)
            } else {
                setEnglishContent("")
                setHindiContent("")
                setGujratiContent("")
